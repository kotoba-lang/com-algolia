(ns algolia.main
  "Kotodama WASM entrypoint for the Algolia clean-room actor (L4) — Clojure port.
  ...
  facts (`algolia.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [kotoba.lang.text :as str]))

(def ns-prefix "algolia")
(def tier "L4")
...
(def entity-specs
  [{:entity "Index"   :plural "indexes"   :id-prefix "algolia_ind"
    :fields [:name :entries :dataSize :fileSize :lastBuildTimeS :numberOfPendingTasks :pendingTask :virtual]
    :required [:name :entries]
    :coerce {:entries :int :dataSize :int :fileSize :int :lastBuildTimeS :int :numberOfPendingTasks :int :pendingTask :bool :virtual :bool}
    :refs {}}
   {:entity "ApiKey"  :plural "apikeys"  :id-prefix "algolia_api"
    :fields [:value :description :maxHitsPerQuery :maxQueriesPerIPPerHour :validity]
    :required [:value :description]
    :coerce {:maxHitsPerQuery :int :maxQueriesPerIPPerHour :int :validity :int}
    :refs {}}
   {:entity "Synonym" :plural "synonyms" :id-prefix "algolia_syn"
    :fields [:objectID :type :input :word :placeholder :replacements]
    :required [:objectID :type]
    :coerce {}
    :refs {}}
   {:entity "Rule"    :plural "rules"    :id-prefix "algolia_rul"
    :fields [:objectID :description :enabled]
    :required [:objectID :description]
    :coerce {:enabled :bool}
    :refs {}}
   {:entity "Task"    :plural "tasks"    :id-prefix "algolia_tas"
...
(defn emit-facts
  "EAVT facts for one record: {\"algolia.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "algolia-compat" :tier tier :entities entities} 200])
...
(defn start! [] :algolia-compat/ready)
